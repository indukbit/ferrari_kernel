...
#if 0
		pr_err("[FINGER %d]: id = %d, event = %d, x = %d, y = %d\n",
...
static int ft5x46_read_gesture(struct ft5x46_data *ft5x46)
{
	...
	if (buf[0] == FT5X46_GESTURE_DBLCLICK) {
		...
		input_sync(ft5x46->input);
		input_event(ft5x46->input, EV_KEY, KEY_POWER, 0);
		input_sync(ft5x46->input);
	}
...
static int ft5x46_input_enable(struct input_dev *in_dev)
{
	...
	dev_info(ft5x46->dev, "ft5x46 enable!\n");
	ft5x46_resume(ft5x46);
...
{
	int rc = 0;
	struct ft5x46_ts_platform_data *pdata = data->dev->platform_data;
	...
	if (on) {
		...
		gpio_set_value_cansleep(pdata->power_ldo_gpio, 1);
		gpio_set_value_cansleep(pdata->power_ts_gpio, 1);
	} else {
		gpio_set_value_cansleep(pdata->power_ts_gpio, 0);
		gpio_set_value_cansleep(pdata->power_ldo_gpio, 0);
...
static int ft5x46_power_init(struct ft5x46_data *data, bool on)
{
	...
	if (gpio_is_valid(pdata->power_ldo_gpio)) {
		rc = gpio_request(pdata->power_ldo_gpio, "ft5x46_power_ldo_gpio");
		if (!rc) {
			rc = gpio_direction_output(pdata->power_ldo_gpio, 0);
			if (rc) {
				dev_err(data->dev, "Unable to set direction gpio %d\n",
					pdata->power_ldo_gpio);
				goto err_free_power_ldo;
			}
		} else {
			dev_err(data->dev, "Unable to request power ldo gpio %d\n",
				pdata->power_ldo_gpio);
			goto err_free_regulator;
		}
	}

	if (gpio_is_valid(pdata->power_ts_gpio)) {
		rc = gpio_request(pdata->power_ts_gpio, "ft5x46_power_ts_gpio");
		if (!rc) {
			rc = gpio_direction_output(pdata->power_ts_gpio, 0);
			if (rc) {
				dev_err(data->dev, "Unable to set direction gpio %d\n",
					pdata->power_ts_gpio);
				goto err_free_power_ts;
			}
		} else {
			dev_err(data->dev, "Unable to request power ldo gpio %d\n",
				pdata->power_ts_gpio);
			...
			struct ft5x46_ts_platform_data *pdata)
{
	...
	dev_info(dev, "w_max = %d\n", (int)pdata->w_max);
	dev_info(dev, "landing_jiffies = %d\n", (int)pdata->landing_jiffies);
	dev_info(dev, "landing_threshold = %d\n", (int)pdata->landing_threshold);
	dev_info(dev, "staying_threshold = %d\n", (int)pdata->staying_threshold);
	dev_info(dev, "raw min = %d\n", (int)pdata->raw_min);
	dev_info(dev, "raw max = %d\n", (int)pdata->raw_max);
	dev_info(dev, "fw delay 55 ms = %d\n", (int)pdata->ui.delay_55);
	dev_info(dev, "fw delay aa ms = %d\n", (int)pdata->ui.delay_aa);
	dev_info(dev, "fw upgrade id 1 = %d\n", (int)pdata->ui.upgrade_id_1);
	dev_info(dev, "fw upgrade id 2 = %d\n", (int)pdata->ui.upgrade_id_2);
	dev_info(dev, "fw delay readid ms = %d\n", (int)pdata->ui.delay_readid);

	for (j = 0; j < pdata->cfg_size; j++) {
		dev_info(dev, "firmare %d chip = 0x%x, vendor = 0x%x name = %s\n",
			...
			struct ft5x46_ts_platform_data *pdata)
{
	...
	pdata->power_ldo_gpio = of_get_named_gpio_flags(np, "ft5x46_i2c,power-ldo-gpio",
				0, &pdata->power_ldo_gpio_flags);
	if (pdata->power_ldo_gpio < 0)
		return pdata->power_ldo_gpio;

	pdata->power_ts_gpio = of_get_named_gpio_flags(np, "ft5x46_i2c,power-ts-gpio",
				0, &pdata->power_ts_gpio_flags);
	if (pdata->power_ts_gpio < 0)
		return pdata->power_ts_gpio;
	...
	pdata->firmware = kmalloc(sizeof(struct ft5x46_firmware_data) * (num_fw + 1),
				GFP_KERNEL);
	if (pdata->firmware == NULL)
		return -ENOMEM;
	pdata->testdata = kmalloc(sizeof(struct ft5x46_test_data) * num_fw, GFP_KERNEL);
				...
				const struct ft5x46_bus_ops *bops)
{
	...
	input_set_capability(ft5x46->input, EV_KEY, KEY_POWER);
...
err_pinctrl_init:
	if (ft5x46->ts_pinctrl)
		ft5x46_pinctrl_select(ft5x46, false);
err_power:
	if (pdata->power_on)

...
#include <linux/i2c.h>
#include <linux/i2c/atmel_mxt_ts_336t.h>
...
start:
	...
	if (data->panel_id == 0) {
		dev_err(dev, "WAD: No lockdown info stored, check the first byte of config info\n");
		if (data->config_info[0] == 0 || data->config_info[0] == 1) {
			data->panel_id = 0x33;
			dev_info(dev, "WAD: old user id = %d, Wintek Panel\n",
					data->config_info[0]);
		} else {
			dev_info(dev, "WAD: Recognize as Biel panel\n");
			data->panel_id = 0x31;	/* Biel Panel */
		}
	}
...
static void mxt_enable_gesture_mode(struct mxt_data *data, u8 enable)
{
	...
	if (error)
		dev_info(&data->client->dev, "write to t93 enabled failed!\n");
}
...
static int mxt_chip_reset(struct mxt_data *data)
{
	...
	mxt_disable_irq(data);
	gpio_set_value(data->pdata->power_ts_gpio, 0);
	msleep(20);
	gpio_set_value(data->pdata->power_ts_gpio, 1);
...
static void mxt_set_gesture_wake_up(struct mxt_data *data, bool enable)
{
	...
	t61_val1 = enable ? 0x50 : 0x88;
	t61_val2 = enable ? 0xC3 : 0x13;
...
static void mxt_dump_value(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	dev_info(dev, "power ldo gpio= %d\n", pdata->power_ldo_gpio);
	dev_info(dev, "power ts gpio= %d\n", pdata->power_ts_gpio);
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	pdata->power_ldo_gpio = of_get_named_gpio_flags(np, "atmel,power-ldo-gpio",
				0, &pdata->power_ldo_gpio_flags);
	pdata->power_ts_gpio = of_get_named_gpio_flags(np, "atmel,power-ts-gpio",
		...
		const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(pdata->power_ldo_gpio) && gpio_is_valid(pdata->power_ts_gpio)) {
		error = gpio_request(pdata->power_ldo_gpio, "mxt_gpio_power_ldo");
		if (!error) {
			error = gpio_direction_output(pdata->power_ldo_gpio, 1);
			if (error) {
				dev_err(&client->dev, "unable to set direction gpio %d\n",
					pdata->power_ldo_gpio);
				goto err_free_power_gpio;
			}
		} else {
			dev_err(&client->dev, "unable to request power ldo gpio %d\n",
				pdata->power_ldo_gpio);
			goto err_pinctrl_sleep;
		}

		error = gpio_request(pdata->power_ts_gpio, "mxt_gpio_power_ts");
		if (!error) {
			error = gpio_direction_output(pdata->power_ts_gpio, 1);
			if (error) {
				dev_err(&client->dev, "unable to set direction gpio %d\n",
					pdata->power_ts_gpio);
				goto err_free_power_gpio;
			...
			dev_err(&client->dev, "unable to request power ts gpio %d\n",
				pdata->power_ts_gpio);
			goto err_free_power_gpio;
		}
	}
	...
	if (error)
	{
		dev_err(&client->dev, "power ldo gpio = %d\n", (int)gpio_get_value(pdata->power_ldo_gpio));
		dev_err(&client->dev, "power ts gpio = %d\n", (int)gpio_get_value(pdata->power_ts_gpio));
